...
void GameOverControl(bool* canHold, bool* isCreated);
...
void ControlTetris(void) {
	...
	static bool canMoveLeft[4] = { false,false,false,false };
	static bool canMoveRight[4] = { false,false,false,false };
	...
	if (GameScene) {
		...
		if (SetDown) {
			...
			if (EffectOn) {
				...
				EffectRoop++;
				if (EffectRoop > EFFECTTIME) {//ループさせたときに落下物消えないバグ発生
					EffectRoop = 0;
				}
				if (!EffectRoop) {
					canupChain = true;
	...
	//GAME OVERの処理//
	GameOverControl(&canHold,&isCreated);			
}

void GameOverControl(bool* canHold,bool* isCreated) {
	for (int CheckGOHeight = 1; CheckGOHeight < FIELD_HEIGHT - 1; CheckGOHeight++) {
		for (int CheckGOWidth = 1; CheckGOWidth < FIELD_WIDTH - 1; CheckGOWidth++) {
